...
ACL.prototype.create_user = function (uid, cb) {
  this.cypher('create (u:User { uid: {uid} }), (g:Group {name: "/"}), (u)-[:OWNS]->(g) return id(u) as uid, id(g) as gid', {uid: uid}, function (err, res) {
    ...
    var rval = {uid: res.data[0][0], gid: res.data[0][1]};
...
ACL.prototype.delete_user = function (uid, cb) {
  this.cypher('match (u:User {uid: {uid} }) optional match (u)-[r:OWNS]->(x) delete r,u,x return true', {uid: uid}, function (err, res) {
    if (err) {
      return cb(err);
    }
    debug("DELETE user returned: " + res);
    var rval = res.data[0][0];
    cb(null, rval);
  });
};
