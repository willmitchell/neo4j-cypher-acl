...
var ACL = function (config) {
  GLOBAL_CONFIG_HACK = this.config = config;
};
...
class QueryTemplate {
  ...
  marshal() {
    let url = GLOBAL_CONFIG_HACK.neo4j.root_url + GLOBAL_CONFIG_HACK.neo4j.path;
    let self = this;
    var cb = function (err, res) {
      self.unmarshal(err, res);
    };
 ...
 * @returns {Request}
 */
ACL.prototype.cypher = function (query, params, cb) {
  return new QueryTemplate(cb, query, params).go();
};
...
ACL.prototype.warning_delete_all_nodes_and_edges = function (cb) {
  ...
  var qt = new QT(cb, 'MATCH (n) OPTIONAL MATCH (n)-[r]-() DELETE n,r return count(n) as count', {}, "DELETEALL");
  qt.go();
};
...
ACL.prototype.create_user = function (uid, cb) {
  this.cypher('create (u:User { uid: {uid} }), (g:Group {name: "/"}), (u)-[:OWNS]->(g) return id(u) as uid, id(g) as gid', {uid: uid}, function (err, res) {
    if (err) {
      return cb(err);
    }
...
ACL.prototype.list_connections = function (uid, cb) {
  var query = "match (u:User {uid: {uid}})-[:CONN]->(c:Connection {key: {key}})<-[:CONN]-(u2:User) return c.data, u2.data";
