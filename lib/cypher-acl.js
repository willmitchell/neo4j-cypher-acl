...
/**
 * Create an Org (Organization) that is owned by the specified User.
 *
 * @param uid the UID of a User node in Neo.
 * @param name the name of the Organization to create.
 * @param cb
 */
ACL.prototype.create_org = function (uid, name, cb) {
  this.cypher('match (u:User {uid: {uid} }) merge (o:Org {name: {name}}) merge (u)-[:OWNS]->(o) return id(o) as oid', {name: name}, function (err, res) {
    if (err) {
      return cb(err);
    }
    debug("CREATING org returned: " + JSON.stringify(res));
...
/**
 * Add an Asset.  This method associates the Asset with the specified User (via uid).
 *
 * @param uid the UID of a User node in Neo.
 * @param aid an external id for an asset.
 * @param cb
 */
ACL.prototype.add_asset = function (uid, aid, cb) {
  this.cypher('match (u:User {uid: {uid}}) merge (a:Asset {aid: {aid}}) merge (u)-[:OWNS]->(a) return id(a) as aid', {uid: uid, aid: aid}, function (err, res) {
    if (err) {
      return cb(err);
    }
...
ACL.prototype.put_asset_in_group = function (uid, aid, gname, cb) {
  this.cypher('match (u:User {uid: {uid} }) create (g:Group {name: {name}})', {name: gname}, function (err, res) {
    if (err) {
      return cb(err);
    }
    debug("CREATING group returned: " + JSON.stringify(res));
...
ACL.prototype.delete_user = function (uid, cb) {
  this.cypher('match (u:User {uid: {uid} })-[r:OWNS_ACCT]->(a:Account) delete u,r,a return true', {uid: uid}, function (err, res) {
    if (err) {
      return cb(err);
    }
...
ACL.prototype.list_connections_for_user = function(uid, cb) {
  var query = "match (u:User {uid: {uid}})-[:CONN]->(c:Connection)<-[:CONN]-(u2:User) return id(u),id(c),id(u2),c.data";
  var params = {uid: uid};
