...
ACL.prototype.get_user = function (uid, cb) {
  var query = 'match (u:User {uid: {uid}}) optional match (g:Group), (u)-[:OWNS]-(g) return u as user';
  var params = {uid: uid};
  return this.q({
    query: query,
    params: params,
...
ACL.prototype.create_group = function (uid, name, cb) {
  return this.q({
    query: 'match (u:User {uid: {uid} })-[r:HAS_GROUP]->(g:Group {name:"/"}) merge (g)-[:HAS_CHILD]->(g2:Group {name: {name}}) return id(g2) as gnid',
    params: {
      uid,
      name
    },
...
ACL.prototype.create_asset = function (uid, aid, group_name, cb) {
  ...
  return this.q({
    query: 'merge (u:User {uid: {uid}})-[r:HAS_GROUP]->(g:Group {name: {name}}) ' +
...
ACL.prototype.list_assets = function (uid, aid, group_name, cb) {
  if (group_name === undefined) {
    debug("Warning: no group name specified in call to add_asset, assuming root.");
    group_name = "/";
  }
